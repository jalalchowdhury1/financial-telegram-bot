...
def main():
    ...
    try:
        ...
        html_content = ""
        # The first non-empty part is metadata, the second is usually the HTML.
        # We'll scan all parts and pick the one that looks most like the Redfin listing page.
        for part in parts:
            ...
            # Heuristic: the real listing page is large and contains Redfin specific tags
            if "data-rf-test-id" in body and len(body) > len(html_content):
                html_content = body
        
        if not html_content:
            # Fallback to the second part if heuristic fails
            if len(parts) > 2:
        ...
        # Debug: print a slice where we expect the price
        ...
        print(f"Price updated in {HISTORY_FILE}")
